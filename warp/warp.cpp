...
int main(int argc, const char * argv[]) {
    if ( argc != 7 ) {
        ...
        printf( "usage: %s -INDIR -EMBED_SIZE -LR -MAX_ITER -N -OUTDIR\n", argv[0] );
    }
    else {
        const char *indir = argv[1]; //input file
        embed_size = atoi(argv[2]); //dimensionality of embeddings
        lr = atof(argv[3]); //learning rate
        max_iter = atoi(argv[4]); //maxmimum iterations
        num_threads = atoi(argv[5]); // number of threads
	const char *outdir = argv[6];
        char filename[BUFFER_SIZE];
        
        time_t t;
        srand((unsigned) time(&t));//tmp filename for different files
        
        /* Load number of features, types, and mentions */
        snprintf(filename, sizeof(filename), "%s/feature.txt", indir); //feature id count
        feature_count = count_lines(filename);
        snprintf(filename, sizeof(filename), "%s/type.txt", indir); // type id freq
        type_count = count_lines(filename);
        snprintf(filename, sizeof(filename), "%s/mention.txt", indir); // mentions id
        mention_count = count_lines(filename);
        printf("type: %d, feature: %d, mention: %d\n",type_count, feature_count, mention_count);//print out total numbers
        ...
        snprintf(filename, sizeof(filename), "%s/mention_feature.txt", indir);
        ...
        }//initialize the input feature vectors
        load_data(filename, train_x, x_count); //load in the mention-feature matrix
        snprintf(filename, sizeof(filename), "%s/mention_type.txt", indir);
        ...
        }//initialize mention-type matrix
        load_data(filename, train_y, y_count); //load in mention-type matrix
        ;
        ...
        {
            error = 0;
            for (a = 0; a < num_threads; a++) pthread_create(&pt[a], NULL, train_BCD_thread, (void *)a); //create threads
            for (a = 0; a < num_threads; a++) pthread_join(pt[a], NULL);
            printf("Iter:%d, error:%f\n",iter, error);
        }
        /* Save matrix A and B*/
        snprintf(filename, sizeof(filename), "%s/warp_A.txt", outdir);
        print_matrix(filename, A, feature_count, embed_size);
        snprintf(filename, sizeof(filename), "%s/warp_B.txt", outdir);
